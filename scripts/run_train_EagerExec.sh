...
OUTPUT_PATH=${TRAIN_CODE_PATH}/OUTPUTS_769_eager_contrast_spc_focal_spc
...
/work/scratch/chendi/anaconda3/envs/py3.8/bin/python "${TRAIN_CODE_PATH}"/train_EagerExec.py --out_path="${OUTPUT_PATH}"/ckpt --searched_with=focal_spc --ckpt_name=/home/students/chendi/projects/Auto-DeepLab-main/OUTPUTS_769_s_focal_spc_r_focal/ckpt/device/autodeeplab-paral_2-100_743.ckpt --crop_size=769 --save_epochs=1 --data_path="${DATASET_PATH}" --modelArts=False --parallel=False --batch_size=4 --start_epoch=1200 --criterion=contrastive_focal --affine=False --epochs="${EPOCHS}" >>log_contrast_focal_spc_focal_spc_debug.txt 2>&1
