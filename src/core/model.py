...
from .encoder import get_default_arch, get_searched_arch, Encoder
...
class AutoDeepLab(nn.Cell):
    ...
    def __init__(self, args=None):
        ...
        if args.searched_with is not None:
            network_arch, cell_arch = get_searched_arch(args.searched_with)
        else:
    ...
    def construct(self, x):
        ...
        if self.criterion == 'ohemce' or self.criterion == 'focal':
            return output
        elif self.criterion == 'analysis':
            return output, self.resize_bilinear(low_level_feature, (x.shape[2], x.shape[3]), None, True)
        else:
            normalize_ops = ops.L2Normalize(axis=1)
            emb = normalize_ops(self.resize_bilinear(
                nn.Conv2d(in_channels=low_level_feature.shape[1], out_channels=32, kernel_size=1)(low_level_feature),
                (x.shape[2], x.shape[3]), None, True))
            return output, emb
