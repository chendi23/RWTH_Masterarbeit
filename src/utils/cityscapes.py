...
def CityScapesDataset(mindrecord_file, process_option='train', ignore_label=255, crop_size=(769, 769),
    ...
    elif process_option == 'infer':
        dataset = ds.MindDataset(mindrecord_file, columns_list=['image'],
                                 num_parallel_workers=2, shuffle=False)
        preprocess = normalize_img_only
        dataset = dataset.map(operations=preprocess, input_columns=['image'],
                              output_columns=["image"], num_parallel_workers=4)
        return dataset
    else:
