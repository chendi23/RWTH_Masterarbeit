...
class PixelContrastLoss(nn.Cell):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):
        ...
        for ii in range(batch_size):
            this_y = y_hat[ii]
            this_classes = ops.Unique()(this_y)[0]
            this_classes = stop_gradient(this_classes)
            this_classes = [this_classes[x] for x in range(this_classes.shape[0]) if
                            this_classes[x] != self.ignore_label]
            this_classes = [this_classes[x] for x in range(len(this_classes)) if
                            (this_y == this_classes[x]).nonzero().shape[0] > self.max_views]
        ...
        # X_ = np.zeros((total_classes, n_view, feat_dim), np.float32)
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:
                ...
                perm = ops.Randperm(max_length=max(num_hard, 1), pad=-1)(ops.Tensor([num_hard], mindspore.int32))
                # perm = np.random.permutation(num_hard)
                # perm = ops.Tensor(perm)
                hard_indices = hard_indices[perm[:num_hard_keep]]

                if num_easy_keep != 0:
                    perm = ops.Randperm(max_length=max(num_easy, 1), pad=-1)(ops.Tensor([num_easy], mindspore.int32))
                ...
                m_mask = ops.logical_and(indices < 769 * 769, indices > 0)
                m_mask = ops.stop_gradient(m_mask)
                indices = indices * m_mask


                X_[X_ptr] = ops.scatter_update(input_x=X_[X_ptr], updates=X[ii, indices].squeeze(1),
                                               indices=ops.arange(indices.size))
                y_[X_ptr] = cls_id
                X_ptr += 1
        return X_, y_
    ...
    def _contrastive(self, feats_, labels_):
        ...
        log_prob = logits - ops.log(exp_logits + neg_logits + 1e-1)

        mean_log_prob_pos = (mask * log_prob).sum(1) / (mask.sum(1) + 1e-10)
        loss = - (self.temperature / self.base_temperature) * mean_log_prob_pos
        loss = loss.mean()
        loss = ops.abs(loss + 1e-3)
        loss = min(loss, 5)
        return loss
...
class Contrast_Focal(nn.Cell):
    def __init__(self, args):
        super(Contrast_Focal, self).__init__()
        self.contrast_loss = PixelContrastLoss(args)
    ...
    def construct(self, emb, labels, out):
        contrast_part = self.contrast_weight * self.contrast_loss(emb, labels, out)
        focal_part = self.focal_loss(out, labels)
        total_loss = contrast_part + focal_part
        return total_loss
...
def build_criterion(args):
    ...
    if args.criterion == 'ce':
        ...
        loss = FocalLoss()
    elif args.criterion == 'contrastive_focal':
        loss = Contrast_Focal(args)
    else:
