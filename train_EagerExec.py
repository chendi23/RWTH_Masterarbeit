...
def train():
    ...
    # model
    class Custom_ContrastiveLossNet(nn.Cell):
        def __init__(self, backbone, loss_f):
            super(Custom_ContrastiveLossNet, self).__init__(auto_prefix=False)
            self._backbone = backbone
            self._loss_fn = loss_f

        def construct(self, data, label):
            output, emb = self._backbone(data)
            # print(output.shape, emb.shape)
            return self._loss_fn(emb, label, output)

    instance_Custom_ContrastiveLossNet = Custom_ContrastiveLossNet(net, criterion)
    model = Model(instance_Custom_ContrastiveLossNet, loss_scale_manager=loss_scale_manager, optimizer=optimizer)
