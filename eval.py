...
def evaluate():
    ...
    if int(args.get_embeddings_eval) == 1:
        include_embeds = True
    # load checkpoint
    param_dict = load_checkpoint(ckpt_file)
    load_param_into_net(autodeeplab, param_dict)

    net = BuildEvalNetwork(network=autodeeplab, include_embeds=include_embeds)
    net.set_train(False)
    ...
    for _, data in enumerate(eval_ds):
        ...
        for _, scale in enumerate(eval_scale):
            ...
            if int(args.get_embeddings_eval) == 0:
                scaled_pred = net(Tensor(scaled_batch, mindspore.float32))
            else:
                scaled_pred, __ = net(Tensor(scaled_batch, mindspore.float32))
            ...
            if args.eval_flip:
                ...
                if args.criterion != 'analysis':
                    if int(args.get_embeddings_eval) == 0:
                        flip_pred = net(flip_batch)
                    else:
                        flip_pred, flip_embs = net(flip_batch)
                else:
                    flip_pred, ll_feat = net(flip_batch)
    ...
    '''save preds and embs for visualization'''
    if int(args.analysis_preds_embs) == 1:
        save_path_prefix = args.save_path_prefix
        flip_pred = flip_pred.squeeze(0)
        # flip_pred = flip_pred[:, ::32, ::32]
        # np.save(save_path_prefix + args.pred_out_suffix + 'seg.npy', np.argmax(flip_pred, 0).astype(np.int8))
        np.save(save_path_prefix + args.pred_out_suffix + '_origin.npy', np.argmax(flip_pred, 0).astype(np.int8))
        if args.criterion != 'analysis':
            if int(args.get_embeddings_eval) == 1:
                flip_embs = flip_embs.asnumpy().squeeze(0)
                flip_embs = flip_embs[:, ::32, ::32]
                # flip_embs = flip_embs.reshape(flip_embs.shape[0], -1).transpose()
                # np.save(save_path_prefix + args.embs_out_suffix + '.npy', flip_embs)
        else:
            ll_feat = ll_feat.asnumpy().squeeze(0)
            print(ll_feat.shape)
            ll_feat = ll_feat[:, ::32, ::32]
            # np.save(save_path_prefix + '_label' + '.npy', label[-1])
            np.save(save_path_prefix + args.feats_out_suffix + '.npy', ll_feat)
